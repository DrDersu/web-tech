...
<div class="container mb-5">
    ...
    $arr[0]['name']='Ерасыл';
    $arr[0]['surname']='Жумадил';
    $arr[0]['lastname']='Рыстайулы';
    $arr[0]['birthday']='22.09.2000';
    $arr[0]['inn']='444';
    $arr[0]['place']='г. Шымкент';
    $arr[0]['wife']='холост';
    $arr[0]['order']='курсант';
    $arr[0]['education']='среднее';
    $arr[0]['nation']='казах';
    $arr[0]['special']='КБ';
    $arr[0]['military']='Есть';
    $arr[0]['sports_category']='Нет';
    $arr[0]['postponement']='Учеба';
    $arr[0]['conviction']='Нет';
    $arr[0]['email']='87778887788';
    ...
    if ($ses==1){
        ...
        <div class="form-group">
            <label for="inn2" class="btn-outline-dark  font-weight-bold " style="font-size: 22px">ИИН студента</label>
            <input type="text" name="inn2" class="form-control" value=" ">
        </div>
        <div class="form-group">
                <label for="Email" class="btn-outline-dark  font-weight-bold " style="font-size: 22px">Email или телефон</label>
                <input type="text" name="Email" class="form-control" value=" ">
                <button class="btn btn-outline-light mt-3" onclick="<?php $inn=$_GET['inn2'] ?>">Открыть</button>
        </div>
        </form>
    ...
    <form method="get">
        <div class="form-group">
            <label for="code" class="btn-outline-dark  font-weight-bold " style="font-size: 22px">Код подтверждение</label>
            <input type="hidden" name="inn2" class="form-control" value="444">
            <input type="text" name="code" class="form-control" value=" ">
            <button class="btn btn-outline-light mt-3" onclick="<?php $code=$_GET['code'] ?>">Открыть</button>
        </div>
    </form>
    ...
    if($ses==3){
?>
        <a href="download2.php?filename=sprvk.docx" class="btn btn-lg btn-info font-weight-bold ml-5" role="button" aria-pressed="true">Скачать
            справку.
        </a>
